...
class RedisClient(object):
    def __init__(self, host=REDIS_HOST, port=REDIS_PORT):
        if REDIS_PASSWORD:
            self._db = Redis(host=host, port=port, db=2, password=REDIS_PASSWORD)
        else:
            self._db = Redis(host=host, port=port, db=2)
...
class MongoDB(object):
    def __init__(self,host=MONGO_HOST,port=MONGO_PORT):
        ...
        self._db = self._client['wechat']
        self._collection = self._db['wechat'] 
